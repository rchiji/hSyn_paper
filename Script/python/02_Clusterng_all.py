...
umap_df = joblib.load("pkl/umap_df_20250815.pkl")
umap_result = joblib.load("pkl/umap_model_20250815.pkl")


# Clustering
OUTDIR = "02_Clustering/leiden_pipeline_outs/all"
os.makedirs(OUTDIR, exist_ok=True)
K = 20                       # number of neighbors
USE_COSINE = True            # cosine similarity via inner product
NORMALIZE_FOR_COSINE = True  # L2-normalize features before IP search
HNSW_M = 32                  # number of neighbors - HNSW
HNSW_EF_CONSTRUCT = 200
HNSW_EF_SEARCH = 128
RESOLUTIONS = [0.1, 0.2, 0.4, 0.6]  # resolution
SEED = 123
random.seed(SEED)
np.random.seed(SEED)
...
feature_cols = all_data_combined_knee.columns[1:65]
meta_cols = [c for c in all_data_combined_knee.columns if c not in feature_cols]
meta_df = all_data_combined_knee.select(meta_cols)
X = all_data_combined_knee.select(feature_cols).to_numpy()
...
# np.save(os.path.join(OUTDIR, "normalized_features.npy"), X)
...
# joblib.dump(partitions, "pkl/leiden_res_comparison.pkl")


# Visualization
LABELDIR = Path("02_Clustering/leiden_pipeline_outs/all")
...
# joblib.dump(df_all, "pkl/combined_df_umap_label_leiden.pkl")
...
for res in resolutions:
    col = f"cluster_res_{res}"
    n_clust = df_all_random[col].nunique()
    palette = sns.color_palette("tab20", n_colors=n_clust)

    fig, ax = plt.subplots(figsize=(2.5, 2.25))
    fig.subplots_adjust(right=0.82)
    sns.scatterplot(
        data=df_all_random, x="UMAP1", y="UMAP2",
    ...
    out_png = f"01_UMAP/leiden/umap_res{res}_leiden_random.png"
    out_pdf = f"01_UMAP/leiden/umap_res{res}_leiden_random.pdf"
...
plt.savefig("01_UMAP/leiden/umap_by_label.png", dpi=300, bbox_inches="tight")
plt.savefig("01_UMAP/leiden/umap_by_label.pdf", dpi=300, bbox_inches="tight")


# Post Clustering
RES = 0.2
OUT_CLUSTER_TAG = f"Leiden_res{RES}"
OUT_DIR_TXT = f"03_Community/SLICTile/SLICTile_cluster_res{RES}"
os.makedirs(OUT_DIR_TXT, exist_ok=True)

## load Leiden label
lab = pl.read_csv(f"02_Clustering/leiden_pipeline_outs/all/labels_with_meta_res{RES}.tsv", separator="\t")
...
with h5py.File("GNN_InOut.h5", mode="r") as f:
    ...
    for donor in tqdm(donors, desc="Load H5"):
        ...
        features = pd.DataFrame(f[donor]["pred_features"][:], index=idx)  # GNN 64D など
...
df_idx = pd.concat(all_idx, ignore_index=True)  # (N, 2) with Donor, TileID
...
def plot_heatmap(df, color, title_text, out_name, scale_row=True):
...
plot_heatmap(
    ...
    out_name=f"03_Community/Proportion/all/TileComp_{OUT_CLUSTER_TAG}.pdf",
	scale_row=True
)
plot_heatmap(
    ...
    out_name=f"03_Community/Proportion/all/CommComp_{OUT_CLUSTER_TAG}.pdf",
	scale_row=True
)
...
medoid_df.to_csv(f"02_Clustering/leiden_pipeline_res/all/Medoids_{OUT_CLUSTER_TAG}.tsv", sep="\t", index=False)
print(medoid_df.head())

with h5py.File("GNN_InOut.h5", mode="a") as f:
    for donor, d in tqdm(info_dict.items(), desc="Write H5 Leiden"):
        ds_name = OUT_CLUSTER_TAG  # 例: "Leiden_res0.2"
...
def plot_cluster(sample_info_dict, cluster_array, num_cluster,
    ...
    ax1 = plt.subplot(122)
    n_colors = int(np.max(_cluster)) + 1
    palette = sns.color_palette("tab20", n_colors)
    cluster_color_map = {i: palette[i] for i in range(n_colors)}
    for cid in np.unique(_cluster):
        idx = (_cluster == cid)
        if not np.any(idx):
            continue
        point_color = cluster_color_map.get(int(cid), "gray")
        ax1.scatter(
            _df.loc[idx, "x"], _df.loc[idx, "y"],
            s=pt_size,
            color=point_color,
...
out_dir = "03_Community/Map/Map_CellType_Leiden0.2_all"
...
M_tile = (df_ratio_merge_mean_tile
          ...
          .sort_index())
M_tile.to_csv(f"02_Clustering/leiden_pipeline_res/all/Matrix_TileComp_res{RES}.tsv", sep="\t", float_format="%.3f")
M_comm = (df_ratio_merge_mean_comm
          .reindex(columns=order_cols)
          .sort_index())
M_comm.to_csv(f"02_Clustering/leiden_pipeline_res/all/Matrix_CommComp_res{RES}.tsv", sep="\t", float_format="%.3f")
