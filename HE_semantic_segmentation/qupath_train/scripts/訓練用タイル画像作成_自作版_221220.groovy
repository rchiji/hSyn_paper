...
// 左上からタイルを作っていくときに右端と下端の余り部分はタイル画像にならない。余りを考慮してタイルの開始位置を調整
// %で余りを計算。tileSize+tileOverlapで割った時の余りは十分なタイルサイズに満たないので、タイル開始位置をずらす。
...
for(int i=0; i+tileSize<width; i+=nextStart){
    for(int j=0; j+tileSize<height; j+=nextStart){
        coordList << [i,j]
    }
}
print coordList
...
// 左上からタイルを作っていくときに右端と下端の余り部分はタイル画像にならない。余りを考慮してタイルの開始位置を調整
if(adjustCenter) {    
    int leftStart = Math.round((width - (coordList[-1][0]+tileSize))/2) 
    int topStart = Math.round((height - (coordList[-1][1]+tileSize))/2)
    print "The remainder px (left/right: ${leftStart}, top/bottom: ${topStart}) could not be included in tiles"
    
    coordList = []
    for(int i=leftStart; i+tileSize<width; i+=nextStart){
        for(int j=topStart; j+tileSize<height; j+=tileOverlap){
            coordList << [i,j]
            }
    }
}
...
tileList.each{
    ...
    // --> img["Mask" (-22), 8-bit, 102x102x1x1x1]
