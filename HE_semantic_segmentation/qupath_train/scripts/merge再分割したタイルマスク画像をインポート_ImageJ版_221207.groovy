...
// マスク画像が入っているフォルダへのパス
def labelDir = buildFilePath(PROJECT_BASE_DIR, 'NNMT_predict')
def extension = ".png"
// アノテーションクラス名。輝度値順と揃えておく必要アリ。
def className = ["Tumor","ilet", "Acinar","Immune cells","Stroma", "Tumor_Duct", "Neuron", "Duct", "Necrosis", "GerminalCenter", "Vessel"]

// ダウンサンプリングしていれば要変更
def downsample = 5
// RoiManager
// https://javadoc.scijava.org/ImageJ1/ij/ij/plugin/frame/RoiManager.html
def saveDir = buildFilePath(PROJECT_BASE_DIR, 'Pred_masks_RoiManager')
// --------------------------------
