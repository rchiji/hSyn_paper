...
all_data_combined_knee_lining = all_data_combined_knee.filter(
    pl.col("label").is_in(["lining"])
)



# Dimensionality_reduction
feature_columns = all_data_combined_knee_lining.columns[1:65]
features = all_data_combined_knee_lining.select(pl.col(feature_columns)).to_numpy()
...
tile_ids = all_data_combined_knee_lining[:, 0].to_numpy()
donors = all_data_combined_knee_lining[:, 65].to_numpy()
pca_df = pd.DataFrame(pca_result, columns=[f"PC{i+1}" for i in range(pca_result.shape[1])])
pca_df["TileID"] = tile_ids
pca_df["Donor"] = donors

joblib.dump(pca_df, "03_python_outs/pkl/pca_df_lining_20260318.pkl")
joblib.dump(pca_result, "03_python_outs/pkl/pca_model_lining_20260318.pkl")

pca_result_select = pca_result[:, :7]
...
joblib.dump(umap_df, "03_python_outs/pkl/umap_df_lining_20260318.pkl")
joblib.dump(umap_result, "03_python_outs/pkl/umap_model_lining_20260318.pkl")



# Clustering
OUTDIR = "03_python_outs/Clustering/leiden_pipeline_outs/lining"
...
feature_columns = all_data_combined_knee_lining.columns[1:65]
meta_cols = [c for c in all_data_combined_knee_lining.columns if c not in feature_columns]
meta_df = all_data_combined_knee_lining.select(meta_cols)

X = all_data_combined_knee_lining.select(feature_columns).to_numpy()
...
np.save(os.path.join(OUTDIR, "normalized_features_lining.npy"), X)
...
# -> Feature matrix: (349156, 64), dtype=float32
# -> (201631, 20) int64 (349156, 20) float32
# -> Has self neighbor? False
# -> sim min/max: 0.08378010988235474 0.9998210072517395
...
# Visualization
## UMAP
LABELDIR = Path("03_python_outs/Clustering/leiden_pipeline_outs/lining")
...
joblib.dump(df_all, "03_python_outs/pkl/combined_df_umap_label_leiden_lining.pkl")
...
col = f"cluster_res_0.1"
turbo9_hex = ["#2E1A47", "#1F3C88", "#0B6E4F", "#1B998B", "#F4A261", "#E76F51", "#D62828", "#8E2D56", "#3A0CA3"]
palette = {0: turbo9_hex[0], 1: turbo9_hex[1], 2: turbo9_hex[2], 3: turbo9_hex[3], 4: turbo9_hex[4], 5: turbo9_hex[5], 6: turbo9_hex[6], 7: turbo9_hex[7], 8: turbo9_hex[8]}
...
out_png = f"99_Fig/fig5/umap_res0.1_leiden_random_lining.png"
out_pdf = f"99_Fig/fig5/umap_res0.1_leiden_random_lining.pdf"
...
plt.savefig("99_Fig/fig5/umap_by_label_lining.png", dpi=300, bbox_inches="tight")
plt.savefig("99_Fig/fig5/umap_by_label_lining.pdf", dpi=300, bbox_inches="tight")
plt.close(fig)


## Community
RES = 0.1
OUT_CLUSTER_TAG = f"Lining_Leiden_res{RES}" 
OUT_DIR_TXT = f"03_python_outs/Community/SLICTile/SLICTile_cluster_res{RES}_lining"
os.makedirs(OUT_DIR_TXT, exist_ok=True)

lab = pl.read_csv(f"03_python_outs/Clustering/leiden_pipeline_outs/veliningssel/labels_with_meta_res{RES}.tsv", separator="\t")
...
def plot_cluster(sample_info_dict, cluster_array, num_cluster,
    ...
    ax1 = plt.subplot(122)
    cluster_color_map = {0: turbo9_hex[0], 1: turbo9_hex[1], 2: turbo9_hex[2], 3: turbo9_hex[3], 4: turbo9_hex[4], 5: turbo9_hex[5], 6: turbo9_hex[6], 7: turbo9_hex[7], 8: turbo9_hex[8]}
...
out_dir = "99_Fig/fig5/Map_CellType_Leiden0.1_lining"
